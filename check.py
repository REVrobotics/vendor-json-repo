...
json_schema = {
        ...
        'name': '',
        'version': '',
        # 'wpilibYear': '', # wpilibYear was frcYear, removed from schema check so it can be checked later
        'uuid': '',
        'mavenUrls': [''],
        'jsonUrl': '',
        'requires': Optional([{
...
def check_schema(j, schema, key):
    ...
    if isinstance(j, dict):
        for k in j:
            if k not in schema:
                if k != "wpilibYear" and k != "frcYear": # wpilibYear & frcYear are special cases that we check later, so allow them to be present even though they're not in the schema
